...
# Detects amd64 vs arm64 and pulls the matching binary from the agentbeta release.
#
# Usage:
...
if [ -z "$BACKEND_URL" ]; then
  echo "beacle: pass backend URL:" >&2
  echo "  curl -fsSL .../install_agent.sh | sudo bash -s http://100.x.x.x:9930" >&2
  exit 1
fi
...
ARCH="$(uname -m)"
case "$ARCH" in
  aarch64|arm64) ASSET="beacle-agent-arm64" ;;
  x86_64|amd64)  ASSET="beacle-agent-amd64" ;;
  *)
...
echo "[beacle] installing agent ($ARCH -> $ASSET) to $INSTALL_DIR"
mkdir -p "$INSTALL_DIR/versions"

echo "[beacle] downloading $BASE/$ASSET"
curl -fsSL "$BASE/$ASSET" -o "$INSTALL_DIR/beacle-agent.new"
